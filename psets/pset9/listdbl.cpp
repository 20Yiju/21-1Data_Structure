...
void unique(pList p) {
    ...
    for (pNode x = begin(p)->next; x != end(p); x = x->next){
        pNode temp = x -> prev;
...
bool sorted(pList p, bool (*comp)(int a, int b)) {
    ...
    for(pNode x = begin(p) -> next; x != end(p); x = x -> next) {
        if((comp)(x -> data, data)) return false;
        data = x -> data;
    }
