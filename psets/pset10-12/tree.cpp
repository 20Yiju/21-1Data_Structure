...
tree findBT(tree root, int key) {
    ...
    if(root -> left != nullptr){
        tree a = findBT(root -> left, key);
        if(a != nullptr){
            return a;
        }
    }
    if(root -> right != nullptr){
        a = findBT(root -> right, key);
        if(a != nullptr){
            return a;
        }
    }
...
tree trim(tree node, int key) {
    ...
    // node with two childeren: replace it with the successor or predecessor
        ...
        int sh = height(node -> right);
        int ph = height(node -> left);
        if(sh > ph){
    ...
    if (node -> right == nullptr && node -> left == nullptr && node -> key == key) {
        delete node;
        return nullptr;
    }
    if (node -> right != nullptr && node -> left == nullptr && node -> key == key) {
...
int LCA_BTiteration(tree root, tree p, tree q) {
    ...
    vector<int> vecp;
    vector<int> vecq;
    findPathBack(root, p, vecp);
    findPathBack(root, q, vecq);
    for(auto vp : vecp){
        //cout << "\n\tPath: "  << vp << endl;
        for(auto vq : vecq){
            //cout << "\n\tPath: "  << vq << endl;
...
void getkey(tree root, set<int>& s) {
    if (!root) return;
    getkey(root -> left, s);
    s.insert(root -> key);
    getkey(root -> right, s);
}
void setkey(tree root, set<int>& s) {
    if (root == nullptr) return;
    setkey(root -> left, s);
    auto newkey = s.begin();
    root -> key = *newkey;
...
tree trimN(tree root, int N, bool AVLtree) {
    ...
    vector<int> all;
    inorder(root, all);
    assert(size(root) == all.size());
    shuffle(all.data(), all.size());
...
tree buildAVL(int* v, int n) {  // recreation method
    ...
    root -> left = buildAVL(first, mid);
    if (n % 2 != 0)
        root -> right = buildAVL(second, mid);
    else
        root -> right = buildAVL(second, mid-1);
    return root;
}
...
tree reconstruct(tree root) {
    ...
    if (size(root) > 10) {
        vector<tree> v;
        inorder(root, v);     // recycling method
        ...
        vector<int> v;
        inorder(root, v);                // recreation method
