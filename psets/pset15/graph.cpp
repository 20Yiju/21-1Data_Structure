...
void DFS_CCs(graph g) {
	...
	*/
    ...
    g->DFSv = {};                  // clear it not to display,
    queue<int>().swap(g->DFSv);
    DPRINT(cout << "<DFS_CCs\n";);
}
...
void BFSpath(graph g, int v, int w, stack<int>& path) {
    ...
    path = {};  
	stack<int>().swap(path);                 // clear path

	for(int i = w; i != v ;i = g -> parentBFS[i]) {  // push v to w path to the stack path
		path.push(i);
	}
	path.push(v);
